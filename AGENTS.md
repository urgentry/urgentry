...
Org-scoped release deploy, commit, suspect, and commit-file routes validate the control-plane catalog before touching release-adjacent data, so missing org paths fail closed consistently in Tiny and self-hosted mode.
Project-scoped release commits also require a real release-to-project association before they expose the backing org release's commit list.
...
SCIM support is partial today but broader than the original `/Users` slice: `/api/0/organizations/{org}/scim/v2/Users` now mounts list/get/create/patch/delete, and `/api/0/organizations/{org}/scim/v2/Groups` mounts list/get/create/patch/delete with team-backed membership materialization. Compatibility aliases also cover deprecated org `/alert-rules/`, org-level `/monitors/{slug}/` plus `/checkins/`, team-scoped `/external-teams/`, project replay `recording-segments/{segment_id}` and `viewed-by`, Sentry app inventory plus global get/update/delete routes, org installation listing, installation-backed external issue create/delete plus org issue external-issue reads, org preprod artifact `install-details` plus `size-analysis`, project `preprodartifacts/build-distribution/latest`, and org issue `autofix` get/start.
Org monitor list responses aggregate all org projects and now keep populated project refs for each row instead of only returning raw project IDs.
The same org monitor list path now issues one org-scoped monitor-store query instead of walking each project individually.
Prevent repository-management parity is now live in both Tiny and self-hosted control-plane mode on the org-scoped `/api/0/organizations/{org}/prevent/owner/{owner}/...` surface, including repository listing, sync status/start, token listing or rotation, branches, and test analytics reads. Raw Prevent tokens are treated like project tokens: token-bearing reads require `project:tokens:read`, token rotation requires `project:tokens:write`, and browser-session POSTs must satisfy CSRF.
Prevent `test-results` cursor navigation is part of that contract, and slow/flaky suite metrics are computed from the filtered interval in both Tiny and self-hosted mode rather than staying zeroed.
Project and issue event list APIs now keep `per_page` plus any active filters when they emit `Link` headers, and page-two-plus responses advertise a live previous cursor instead of a hard-coded empty one.
Sentry app mutations and installation-backed external issue links now follow the same Tiny versus self-hosted storage seam: SQLite in Tiny mode, Postgres control-plane persistence in serious self-hosted mode. Installation external-issue writes upsert by installation plus issue plus display-name key, and the org issue external-issues read route now returns the Sentry-style `issueId` / `serviceType` / `displayName` / `webUrl` shape.
Preprod artifact parity uses a shared SQLite query-plane read model in both Tiny and serious self-hosted mode. The org install-details and size-analysis endpoints read artifact metadata and optional size-analysis state by artifact ID, while the project latest-build endpoint requires `appId` plus `platform`, returns `{latestArtifact,currentArtifact}`, and validates the `buildVersion` plus `buildNumber|mainBinaryIdentifier` query contract with explicit `400` responses.
Issue autofix parity also uses a shared SQLite query-plane run log in both modes. `GET /api/0/organizations/{org}/issues/{issue_id}/autofix/` returns `{"autofix": null}` until a run exists, and `POST` returns `202` plus a numeric `run_id` after persisting a durable experimental autofix state payload keyed to the target issue.
Org release-list reads batch native summary lookups through the shared SQLite native-control path, so Tiny and self-hosted release lists no longer pay one native-summary query per listed version.
Org member-list reads batch team-slug lookups for the whole org before assembling each row, so Tiny and self-hosted control-plane responses keep the same `teams` array without walking `ListUserTeams` per member.
