...
func runProfile(args []string) {
	...
	scenario := fs.String("scenario", "store-basic-error", "profile scenario: store-basic-error|store-python-full|envelope-single-error|otlp-trace")
	kind := fs.String("kind", string(profile.KindNone), "profile kind: none|cpu|heap|allocs|trace")
	iterations := fs.Int("iterations", 200, "number of fixed payload iterations")
	outDir := fs.String("out-dir", "profiles/latest", "directory for summary, profile artifact, and captured data")
	projectID := fs.String("project-id", "profile-project", "project id used during profiling runs")
	gomaxprocs := fs.Int("gomaxprocs", 1, "GOMAXPROCS to pin for deterministic runs")
	if err := fs.Parse(args); err != nil {
		log.Fatal().Err(err).Msg("parse profile flags")
	}

	logging.Setup("profile")
	if err := profile.Run(profile.Config{
		Scenario:   *scenario,
		...
		log.Fatal().Err(err).Msg("profile failed")
	}
}
