...
create_env_file() {
  ...
  cp -f "$ENV_TEMPLATE" "$ENV_FILE"
  {
    ...
    echo "POSTGRES_PASSWORD=serious-columnar-postgres"
    echo "MINIO_ROOT_PASSWORD=serious-columnar-minio"
    echo "URGENTRY_CONTROL_DATABASE_URL=postgres://\${POSTGRES_USER:-urgentry}:serious-columnar-postgres@postgres:5432/\${POSTGRES_DB:-urgentry}?sslmode=disable"
    echo "URGENTRY_TELEMETRY_DATABASE_URL=postgres://\${POSTGRES_USER:-urgentry}:serious-columnar-postgres@postgres:5432/\${POSTGRES_DB:-urgentry}?sslmode=disable"
    echo "URGENTRY_BOOTSTRAP_PAT=gpat_serious_columnar_smoke"
    echo "URGENTRY_BOOTSTRAP_PASSWORD=SeriousColumnar!123"
    echo "URGENTRY_METRICS_TOKEN=metrics-serious-columnar-smoke"
    echo "CLICKHOUSE_DB=urgentry"
    echo "CLICKHOUSE_USER=urgentry"
    echo "CLICKHOUSE_PASSWORD=SeriousColumnarClickHouse!123"
    echo "URGENTRY_COLUMNAR_DATABASE_URL=clickhouse://urgentry:SeriousColumnarClickHouse!123@clickhouse:9000/urgentry?dial_timeout=30s"
