...
func (s *EventStore) SaveEvent(ctx context.Context, evt *store.StoredEvent) error {
	tagsJSON, summaryJSON, occurredAt := eventWriteFields(evt)
	return withBusyRetry(30*time.Second, func() error {
		stmt, err := s.saveEventStatement(ctx)
		if err != nil {
			return err
		}
		_, err = stmt.ExecContext(ctx,
			...
			string(tagsJSON), string(evt.NormalizedJSON), summaryJSON,
			occurredAt,
			evt.UserIdentifier,
			evt.PayloadKey,
...
func (s *EventStore) saveEventStatement(ctx context.Context) (*sql.Stmt, error) {
	const query = `INSERT OR IGNORE INTO events
				(id, project_id, event_id, group_id, release, environment, platform, level, event_type,
				 title, culprit, message, tags_json, payload_json, summary_json, occurred_at, user_identifier, payload_key, processing_status, ingest_error)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	s.stmtsMu.Lock()
	defer s.stmtsMu.Unlock()
	if s.saveEventStmt != nil {
		return s.saveEventStmt, nil
	}
	stmt, err := s.db.PrepareContext(ctx, query)
	if err != nil {
		return nil, err
	}
	s.saveEventStmt = stmt
	return stmt, nil
}
...
func eventWriteFields(evt *store.StoredEvent) ([]byte, string, string) {
	if evt.EventType == "" {
		evt.EventType = "error"
	}
	...
	return tagsJSON, eventSummaryJSON(evt.NormalizedJSON), evt.OccurredAt.UTC().Format(time.RFC3339)
}

func eventSummaryJSON(payload json.RawMessage) string {
	if len(payload) == 0 {
		return ""
	}
	...
	for _, key := range []string{"exception", "request", "breadcrumbs", "contexts", "sdk", "user", "fingerprint", "errors", "measurements", "packages"} {
