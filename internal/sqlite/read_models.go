...
func ListGroupEventsPaged(ctx context.Context, db *sql.DB, groupID string, limit, offset int) ([]store.WebEvent, error) {
	q := `SELECT event_id, group_id, title, message, level, platform, culprit, occurred_at, tags_json,
	             COALESCE(NULLIF(summary_json, ''), payload_json), COALESCE(processing_status, 'completed'), COALESCE(ingest_error, '')
...
func GetLatestGroupEvent(ctx context.Context, db *sql.DB, groupID string) (*store.WebEvent, error) {
	row := db.QueryRowContext(ctx,
		`SELECT event_id, group_id, title, message, level, platform, culprit, occurred_at, tags_json, COALESCE(NULLIF(summary_json, ''), payload_json),
