...
func (s *GroupStore) GetGroupByKey(ctx context.Context, projectID, version, key string) (*issue.Group, error) {
	stmt, err := s.getGroupByKeyStatement(ctx)
	if err != nil {
		return nil, err
	}
	row := stmt.QueryRowContext(ctx, projectID, version, key)
	return scanGroup(row)
}

func (s *GroupStore) getGroupByKeyStatement(ctx context.Context) (*sql.Stmt, error) {
	const query = `SELECT id, project_id, grouping_version, grouping_key, title, culprit, level, COALESCE(assignee, ''),
				status, resolution_substatus, resolved_in_release, merged_into_group_id, first_seen, last_seen, times_seen, last_event_id
		 FROM groups WHERE project_id = ? AND grouping_version = ? AND grouping_key = ?`
	s.stmtsMu.Lock()
	defer s.stmtsMu.Unlock()
	if s.getGroupByKeyStmt != nil {
		return s.getGroupByKeyStmt, nil
	}
	stmt, err := s.db.PrepareContext(ctx, query)
	if err != nil {
		return nil, err
	}
	s.getGroupByKeyStmt = stmt
	return stmt, nil
}
...
func (s *GroupStore) MergeIssue(ctx context.Context, sourceGroupID, targetGroupID, userID string) error {
	...
	if sourceGroupID == targetGroupID {
		return sharedstore.ErrInvalidIssueWorkflow
	}
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	...
	var sourceProjectID, sourceMergedInto string
	if err := tx.QueryRowContext(ctx,
		`SELECT project_id, COALESCE(merged_into_group_id, '') FROM groups WHERE id = ?`,
		sourceGroupID,
	).Scan(&sourceProjectID, &sourceMergedInto); err != nil {
		if err == sql.ErrNoRows {
			return sharedstore.ErrNotFound
		}
		return err
	}
	var targetProjectID, targetMergedInto string
	if err := tx.QueryRowContext(ctx,
		`SELECT project_id, COALESCE(merged_into_group_id, '') FROM groups WHERE id = ?`,
		targetGroupID,
	).Scan(&targetProjectID, &targetMergedInto); err != nil {
		if err == sql.ErrNoRows {
			return sharedstore.ErrNotFound
		}
		return err
	}
	if sourceProjectID != targetProjectID || targetMergedInto != "" {
		return sharedstore.ErrInvalidIssueWorkflow
	}
	if sourceMergedInto != "" && sourceMergedInto != targetGroupID {
		return sharedstore.ErrInvalidIssueWorkflow
	}
	if sourceMergedInto == targetGroupID {
		return nil
	}
	_, err = tx.ExecContext(ctx,
		`UPDATE groups
...
func (s *GroupStore) UnmergeIssue(ctx context.Context, sourceGroupID, userID string) error {
	...
	if err := tx.QueryRowContext(ctx, `SELECT COALESCE(merged_into_group_id, '') FROM groups WHERE id = ?`, sourceGroupID).Scan(&mergedInto); err != nil {
		if err == sql.ErrNoRows {
			return sharedstore.ErrNotFound
		}
		return err
