...
func TestOpenQueueSupportsDurableJobsWithoutProjectCatalog(t *testing.T) {
	dir := t.TempDir()
	db, err := OpenQueue(dir)
	if err != nil {
		t.Fatalf("OpenQueue: %v", err)
	...
	ok, err := jobs.Enqueue(ctx, JobKindEvent, "proj-missing", []byte(`{"event_id":"evt-queue"}`), 10)
	if err != nil {
	...
	if job.ProjectID != "proj-missing" {
		t.Fatalf("ProjectID = %q, want proj-missing", job.ProjectID)
	}
...
func TestEventStorePersistsEventSummaryJSON(t *testing.T) {
	dir := t.TempDir()
	db, err := Open(dir)
	if err != nil {
		t.Fatalf("Open: %v", err)
	...
	evt := &store.StoredEvent{
		...
		NormalizedJSON: json.RawMessage(`{
			...
			"breadcrumbs":[{"type":"default","message":"step"}],
			"extra":{"ignored":"field"}
		}`),
	}
	...
	var summary string
	if err := db.QueryRowContext(ctx, `SELECT summary_json FROM events WHERE event_id = ?`, "evt-summary-1").Scan(&summary); err != nil {
		t.Fatalf("select summary_json: %v", err)
	}
	if !strings.Contains(summary, `"sdk"`) || !strings.Contains(summary, `"user"`) || !strings.Contains(summary, `"contexts"`) {
		t.Fatalf("summary_json = %s, want selected response fields", summary)
	}
	if strings.Contains(summary, `"extra"`) {
		t.Fatalf("summary_json = %s, did not expect unrelated payload fields", summary)
	}
}
