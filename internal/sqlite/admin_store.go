...
func userRecordByIDTx(ctx context.Context, tx *sql.Tx, userID string) (*auth.User, error) {
	...
	return scanActiveAdminUser(row)
}
...
func scanActiveAdminUser(scanner adminUserScanner) (*auth.User, error) {
	var user auth.User
	if err := scanner.Scan(&user.ID, &user.Email, &user.DisplayName); err != nil {
...
func (s *AdminStore) AddOrgMember(ctx context.Context, orgSlug, userID, role string) (*OrgMemberRecord, error) {
	...
	user, err := userRecordByIDTx(ctx, tx, userID)
	if err != nil {
		return nil, err
	}
	if user == nil {
