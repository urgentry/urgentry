...
func TestNewServiceWithoutBridgeKeepsSQLiteDBForProjectEvents(t *testing.T) {
	t.Parallel()
	...
	service := NewService(db, nil, Dependencies{
		Blobs:       blobs,
		IssueSearch: sqlite.NewWebStore(db),
		Web:         sqlite.NewWebStore(db),
	...
	rows, err := service.ListProjectEvents(ctx, "proj-a", 10, 0)
	if err != nil {
		t.Fatalf("ListProjectEvents: %v", err)
	}
	if len(rows) == 0 {
		t.Fatal("ListProjectEvents returned no rows")
	}
}
