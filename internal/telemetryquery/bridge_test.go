...
type stubColumnarLogs struct {
	items []store.DiscoverLog
}

func (s stubColumnarLogs) ListRecentLogs(_ context.Context, _ string, _ int) ([]store.DiscoverLog, error) {
	return append([]store.DiscoverLog(nil), s.items...), nil
}

func (s stubColumnarLogs) SearchLogs(_ context.Context, _ string, _ string, _ int) ([]store.DiscoverLog, error) {
	return append([]store.DiscoverLog(nil), s.items...), nil
}
...
func TestBridgeListLogsUsesColumnarPilotWhenConfigured(t *testing.T) {
	t.Parallel()
	...
	service := NewService(source, bridge, Dependencies{
		Blobs:        store.NewMemoryBlobStore(),
		IssueSearch:  sqlite.NewWebStore(source),
		ColumnarLogs: stubColumnarLogs{items: []store.DiscoverLog{{EventID: "columnar-log", ProjectID: "proj-a", ProjectSlug: "proj-a", Message: "columnar", Title: "columnar"}}},
		Web:          sqlite.NewWebStore(source),
		Discover:     sqlite.NewDiscoverEngine(source),
		Traces:       sqlite.NewTraceStore(source),
		Replays:      sqlite.NewReplayStore(source, store.NewMemoryBlobStore()),
		Profiles:     telemetrybridge.NewProfileReadStore(bridge, store.NewMemoryBlobStore()),
		Projector:    projector,
	})
	items, err := service.ListRecentLogs(ctx, "acme", 10)
	if err != nil {
		t.Fatalf("ListRecentLogs: %v", err)
	}
	if len(items) != 1 || items[0].EventID != "columnar-log" {
