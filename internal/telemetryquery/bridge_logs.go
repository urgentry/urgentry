...
func (s *bridgeService) listLogs(ctx context.Context, orgSlug, rawQuery string, limit int) ([]store.DiscoverLog, error) {
	...
	if s.columnarLogs != nil {
		...
		if rawQuery == "" {
			items, err = s.columnarLogs.ListRecentLogs(ctx, orgSlug, limit)
		} else {
			items, err = s.columnarLogs.SearchLogs(ctx, orgSlug, rawQuery, limit)
		}
		if err != nil {
			return nil, fmt.Errorf("list columnar logs: %w", err)
		}
