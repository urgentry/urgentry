...
func TestPipeline_RecordsQueueWaitAndFailedProcessingMetrics(t *testing.T) {
	events := &failingEventStore{err: fmt.Errorf("save failed")}
	groups := issue.NewMemoryGroupStore()
	blobs := store.NewMemoryBlobStore()
	proc := &issue.Processor{
		Events: events,
	...
	payload, _ := json.Marshal(map[string]any{
		"event_id":  "ffffffff111122223333444455556666",
		"platform":  "go",
		"level":     "error",
		"message":   "failing metrics path",
		"timestamp": time.Now().UTC().Format(time.RFC3339),
	})
	...
	ok := p.Enqueue(Item{
		ProjectID:  "proj-fail",
		RawEvent:   payload,
		EnqueuedAt: time.Now().Add(-5 * time.Millisecond),
	})
	if !ok {
		t.Fatal("Enqueue returned false")
	}

	waitForPipelineCondition(t, "wait for failed processing metrics", func() error {
		snap := met.Snapshot()
		if snap.Stages[metrics.StageQueueWait].Count < 1 {
			return fmt.Errorf("queue wait count = %d, want >= 1", snap.Stages[metrics.StageQueueWait].Count)
		}
		if snap.Stages[metrics.StageProcessing].Errors < 1 {
			return fmt.Errorf("processing errors = %d, want >= 1", snap.Stages[metrics.StageProcessing].Errors)
		}
		return nil
	})
...
func (s *failingEventStore) SaveEvent(context.Context, *store.StoredEvent) error {
	return s.err
}

func (*failingEventStore) GetEvent(context.Context, string, string) (*store.StoredEvent, error) {
	return nil, nil
}

func (*failingEventStore) ListEvents(context.Context, string, store.ListOpts) ([]*store.StoredEvent, error) {
	return nil, nil
}
