...
import (
	...
	"urgentry/internal/telemetrycolumnar"
...
func effectivePipelineWorkers(cfg config.Config, deployment deploymentMode) int {
	if deployment == deploymentModeTiny && cfg.AppendOnlyIngest {
		return 1
	}
	if cfg.PipelineWorkers > 0 {
		return cfg.PipelineWorkers
	}
	if deployment == deploymentModeSeriousSelfHosted {
		return 4
	}
	return 1
}
...
type runtimeState struct {
	...
	columnarLogs           *telemetrycolumnar.LogStore
...
func (s *runtimeState) openDatabases() error {
	...
	if s.deployment == deploymentModeTiny && !strings.EqualFold(strings.TrimSpace(s.cfg.AsyncBackend), "jetstream") {
		queueDataDir := strings.TrimSpace(s.cfg.QueueDataDir)
		if queueDataDir == "" {
			queueDataDir = "queue"
		}
		if !filepath.IsAbs(queueDataDir) {
			queueDataDir = filepath.Join(s.dataDir, queueDataDir)
		}
		queueDB, err := sqlite.OpenQueue(queueDataDir)
		if err != nil {
			return fmt.Errorf("open queue database: %w", err)
		}
		s.queueDB = queueDB
		log.Info().Str("queue_data_dir", queueDataDir).Msg("queue database ready")
	}
	...
	if strings.TrimSpace(s.cfg.ColumnarDSN) == "" {
		return nil
	}

	columnarBackend := strings.TrimSpace(s.cfg.ColumnarBackend)
	if columnarBackend == "" {
		columnarBackend = string(telemetrycolumnar.BackendClickHouse)
	}
	if !strings.EqualFold(columnarBackend, string(telemetrycolumnar.BackendClickHouse)) {
		return fmt.Errorf("unsupported columnar backend %q", s.cfg.ColumnarBackend)
	}
	columnarDB, err := telemetrycolumnar.Open(context.Background(), s.cfg.ColumnarDSN)
	if err != nil {
		return fmt.Errorf("open columnar telemetry store: %w", err)
	}
	if err := telemetrycolumnar.Migrate(context.Background(), columnarDB); err != nil {
		_ = columnarDB.Close()
		return fmt.Errorf("migrate columnar telemetry store: %w", err)
	}
	s.columnarDB = columnarDB
	s.columnarLogs = telemetrycolumnar.NewLogStore(s.db, columnarDB)
	s.runtimeClosers = append(s.runtimeClosers, columnarDB.Close)
	log.Info().Str("columnar_backend", columnarBackend).Msg("columnar telemetry pilot ready")
	return nil
}
...
func (s *runtimeState) buildCoreStores() error {
	...
	s.jobStore = sqlite.NewJobStore(jobDB)
...
func (s *runtimeState) openAsyncRuntime() error {
	...
	if s.deployment == deploymentModeTiny && s.queueDB != nil && s.cfg.AppendOnlyIngest {
		s.workerQueue = sqlite.NewAppendOnlyEventQueue(s.queueDB, s.jobStore)
	}
	return nil
}
...
func (s *runtimeState) buildRuntimeServices() error {
	...
	if s.telemetryDB != nil {
		s.backfillController.SetTelemetryRebuilder(s.newTelemetryProjector())
	}
	...
	if s.columnarLogs != nil {
		queryDeps.ColumnarLogs = s.columnarLogs
	}
	if s.telemetryDB != nil {
		queryDeps.Profiles = telemetrybridge.NewProfileReadStore(s.telemetryDB, s.blobStore)
		queryDeps.Projector = s.newTelemetryProjector()
	}
	...
	opStore := sqlite.NewOperatorStore(
		...
		store.OperatorRuntime{
			...
			Version:      s.version,
			Region:       s.cfg.Region,
			NodeID:       operatorNodeID(string(s.role)),
...
func (s *runtimeState) newTelemetryProjector() *telemetrybridge.Projector {
	projector := telemetrybridge.NewProjector(s.db, s.telemetryDB)
	if s.columnarLogs != nil {
		projector.SetColumnarLogSink(s.columnarLogs)
	}
	return projector
}

func operatorNodeID(role string) string {
	for _, value := range []string{
		os.Getenv("URGENTRY_NODE_ID"),
		os.Getenv("POD_NAME"),
		operatorHostname(),
		role,
	} {
