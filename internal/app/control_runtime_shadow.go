...
func (s *shadowingAdminStore) FindOrCreateSAMLUser(ctx context.Context, orgID string, user auth.SAMLUser) (*auth.User, error) {
	provisioner, ok := s.base.(auth.SAMLUserProvisioner)
	if !ok {
		return nil, fmt.Errorf("saml user provisioner unavailable")
	}
	return provisioner.FindOrCreateSAMLUser(ctx, orgID, user)
}
