...
func projectIDFromSlugs(r *http.Request, db *sql.DB, orgSlug, projectSlug string) (string, error) {
	if catalog := catalogFromRequest(r); catalog != nil {
		...
		if db != nil {
			org, orgErr := catalog.GetOrganization(r.Context(), orgSlug)
			if orgErr == nil && org != nil {
				if err := ensureSQLiteProjectShadow(r.Context(), db, *org, *project); err != nil {
					log.Warn().Err(err).Str("org_slug", orgSlug).Str("project_slug", projectSlug).Msg("api: failed to refresh sqlite project shadow")
				}
			} else if orgErr != nil {
				log.Warn().Err(orgErr).Str("org_slug", orgSlug).Str("project_slug", projectSlug).Msg("api: failed to load organization for sqlite shadow refresh")
			}
		}
