...
func handleListProjectEvents(db *sql.DB, queries telemetryquery.Service, auth authFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if !auth(w, r) {
			return
		}
		...
		var events []*Event
		if queries != nil {
			rows, qerr := queries.ListProjectEvents(r.Context(), projectID, pg.Limit+1, pg.Offset)
			if qerr == nil {
...
func handleResolveShortID(db *sql.DB, issues controlplane.IssueWorkflowStore, auth authFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		apiIssue, err := loadIssueResponse(r.Context(), db, issues, principalUserID(authPrincipalFromContext(r.Context())), *issue)
		if err != nil {
			httputil.WriteError(w, http.StatusInternalServerError, "Failed to resolve short ID.")
			return
		}
