...
func openTestSQLite(t testing.TB) *sql.DB {
	t.Helper()
...
func seedSQLiteAuth(t testing.TB, db *sql.DB) {
	t.Helper()
...
func saveProfileQueryFixture(t testing.TB, profiles *sqlite.ProfileStore, projectID string, spec profilefixtures.EnvelopeSpec) {
	t.Helper()
	profilefixtures.Save(t, profiles, projectID, spec)
}

func sqliteAuthorizedDependencies(t testing.TB, db *sql.DB, deps Dependencies) Dependencies {
	t.Helper()
	...
	if deps.Prevent == nil {
		deps.Prevent = sqlite.NewPreventStore(db)
	}
...
func newSQLiteTestServer(t testing.TB, db *sql.DB) *httptest.Server {
	t.Helper()
...
func insertSQLiteGroup(t testing.TB, db *sql.DB, id, title, culprit, level, status string) {
	t.Helper()
...
func insertSQLiteEvent(t testing.TB, db *sql.DB, eventID, groupID, title, level string) {
	t.Helper()
	now := time.Now().UTC().Format(time.RFC3339)
...
func TestAPIMergeAndUnmergeIssue_SQLite(t *testing.T) {
	db := openTestSQLite(t)
	seedSQLiteAuth(t, db)
	insertSQLiteGroup(t, db, "grp-merge-source", "Source Issue", "source.go", "error", "unresolved")
	insertSQLiteGroup(t, db, "grp-merge-target", "Target Issue", "target.go", "error", "unresolved")
	if _, err := db.Exec(
	...
	if _, err := db.Exec(
		`INSERT INTO groups (id, project_id, grouping_version, grouping_key, title, culprit, level, status, first_seen, last_seen, times_seen)
		 VALUES ('grp-merge-other', 'test-proj-id-2', 'urgentry-v1', 'merge-other', 'Other Issue', 'other.go', 'error', 'unresolved', ?, ?, 1)`,
		time.Now().UTC().Format(time.RFC3339),
		time.Now().UTC().Format(time.RFC3339),
	); err != nil {
		t.Fatalf("insert foreign group: %v", err)
	}

	ts := newSQLiteTestServer(t, db)
	defer ts.Close()

	crossProject := authzJSONRequest(t, ts, http.MethodPost, "/api/0/issues/grp-merge-source/merge/", "gpat_test_admin_token", map[string]any{
		"targetIssueId": "grp-merge-other",
	})
	if crossProject.StatusCode != http.StatusBadRequest {
		t.Fatalf("cross-project merge status = %d, want 400", crossProject.StatusCode)
	}
	crossProject.Body.Close()

	selfMerge := authzJSONRequest(t, ts, http.MethodPost, "/api/0/issues/grp-merge-source/merge/", "gpat_test_admin_token", map[string]any{
		"targetIssueId": "grp-merge-source",
	})
	if selfMerge.StatusCode != http.StatusBadRequest {
		t.Fatalf("self merge status = %d, want 400", selfMerge.StatusCode)
	}
	selfMerge.Body.Close()

	merge := authzJSONRequest(t, ts, http.MethodPost, "/api/0/issues/grp-merge-source/merge/", "gpat_test_admin_token", map[string]any{
		"targetIssueId": "grp-merge-target",
	})
	if merge.StatusCode != http.StatusOK {
		t.Fatalf("merge status = %d, want 200", merge.StatusCode)
	}
	merge.Body.Close()

	var status, substatus, mergedInto string
	if err := db.QueryRow(
		`SELECT status, COALESCE(resolution_substatus, ''), COALESCE(merged_into_group_id, '') FROM groups WHERE id = 'grp-merge-source'`,
	).Scan(&status, &substatus, &mergedInto); err != nil {
		t.Fatalf("query merged source: %v", err)
	}
	if status != "ignored" || substatus != "merged" || mergedInto != "grp-merge-target" {
		t.Fatalf("unexpected merged source state: status=%q substatus=%q mergedInto=%q", status, substatus, mergedInto)
	}

	var mergeActivities int
	if err := db.QueryRow(`SELECT COUNT(*) FROM issue_activity WHERE group_id = 'grp-merge-source' AND kind = 'merge'`).Scan(&mergeActivities); err != nil {
		t.Fatalf("count merge activity: %v", err)
	}
	if mergeActivities != 1 {
		t.Fatalf("merge activity count = %d, want 1", mergeActivities)
	}

	unmerge := authzJSONRequest(t, ts, http.MethodPost, "/api/0/issues/grp-merge-source/unmerge/", "gpat_test_admin_token", nil)
	if unmerge.StatusCode != http.StatusOK {
		t.Fatalf("unmerge status = %d, want 200", unmerge.StatusCode)
	}
	unmerge.Body.Close()
	...
	if err := db.QueryRow(
		...
		t.Fatalf("query unmerged source: %v", err)
	}
	if status != "unresolved" || substatus != "" || mergedInto != "" {
		t.Fatalf("unexpected unmerged source state: status=%q substatus=%q mergedInto=%q", status, substatus, mergedInto)
	}

	var unmergeActivities int
	if err := db.QueryRow(`SELECT COUNT(*) FROM issue_activity WHERE group_id = 'grp-merge-source' AND kind = 'unmerge'`).Scan(&unmergeActivities); err != nil {
		t.Fatalf("count unmerge activity: %v", err)
	}
	if unmergeActivities != 1 {
		t.Fatalf("unmerge activity count = %d, want 1", unmergeActivities)
	}

	unmergeAgain := authzJSONRequest(t, ts, http.MethodPost, "/api/0/issues/grp-merge-source/unmerge/", "gpat_test_admin_token", nil)
	if unmergeAgain.StatusCode != http.StatusBadRequest {
		t.Fatalf("unmerge non-merged status = %d, want 400", unmergeAgain.StatusCode)
	}
	unmergeAgain.Body.Close()
}
