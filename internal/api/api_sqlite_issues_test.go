...
func BenchmarkAPIListIssues_SQLite(b *testing.B) {
	db := openTestSQLite(b)

	baseTime := time.Date(2026, 4, 11, 12, 0, 0, 0, time.UTC)
	for i := 0; i < 5000; i++ {
		id := "grp-bench-issue-" + strconv.Itoa(i)
		insertSQLiteGroup(b, db, id, "Bench Issue "+strconv.Itoa(i), "main.go", "error", "unresolved")
		if _, err := db.Exec(
			`UPDATE groups SET last_seen = ?, first_seen = ? WHERE id = ?`,
			baseTime.Add(time.Duration(i)*time.Millisecond).Format(time.RFC3339),
			baseTime.Add(-time.Hour).Format(time.RFC3339),
			id,
		); err != nil {
			b.Fatalf("update %s timestamps: %v", id, err)
		}
	}

	ts := newSQLiteTestServer(b, db)
	defer ts.Close()
	client := ts.Client()
	url := ts.URL + "/api/0/projects/test-org/test-project/issues/?per_page=10"
	...
	for i := 0; i < b.N; i++ {
		...
		req.Header.Set("Authorization", "Bearer gpat_test_admin_token")
		...
		if resp.StatusCode != http.StatusOK {
			...
			resp.Body.Close()
			b.Fatalf("status = %d, want 200 body=%s", resp.StatusCode, body)
		}
