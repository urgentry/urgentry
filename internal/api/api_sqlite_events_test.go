...
func BenchmarkAPIListProjectEvents_SQLite(b *testing.B) {
	db := openTestSQLite(b)

	insertSQLiteGroup(b, db, "grp-bench-evt", "BenchEventErr", "main.go", "error", "unresolved")
	timestamp := time.Date(2026, 4, 11, 12, 0, 0, 0, time.UTC)
	for i := 0; i < 5000; i++ {
		eventID := "evt-bench-" + strconv.Itoa(i)
		insertSQLiteEvent(b, db, eventID, "grp-bench-evt", "BenchEventErr", "error")
		if _, err := db.Exec(
			`UPDATE events SET ingested_at = ?, occurred_at = ? WHERE event_id = ?`,
			timestamp.Add(time.Duration(i)*time.Millisecond).Format(time.RFC3339),
			timestamp.Add(time.Duration(i)*time.Millisecond).Format(time.RFC3339),
			eventID,
		); err != nil {
			b.Fatalf("update %s timestamps: %v", eventID, err)
		}
	}
	...
	defer ts.Close()
	client := ts.Client()
	url := ts.URL + "/api/0/projects/test-org/test-project/events/?per_page=10"
