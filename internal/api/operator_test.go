...
func TestAPIOperatorDiagnostics(t *testing.T) {
	...
	if bundle.Summary.Health != "warn" || bundle.Summary.QueueDepth != 5 || bundle.Summary.FailedBackfills != 1 {
		t.Fatalf("unexpected diagnostics summary: %+v", bundle.Summary)
	}
	if bundle.Summary.FleetNodeCount != 1 || len(bundle.FleetNodes) != 1 || bundle.FleetNodes[0].Role != "api" {
		t.Fatalf("unexpected diagnostics fleet nodes: summary=%+v nodes=%+v", bundle.Summary, bundle.FleetNodes)
	}
