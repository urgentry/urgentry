...
func loadIssueResponse(
	...
	extras := loadIssueResponseExtras(ctx, db, issues, userID, []store.WebIssue{row})
	issue := apiIssueFromWebIssueWithExtras(row, extras[row.ID])
	return finalizeLoadedIssueResponse(ctx, db, issue)
}

func finalizeLoadedIssueResponse(ctx context.Context, db *sql.DB, issue Issue) (Issue, error) {
	ref, err := projectRefForIssue(ctx, db, issue.ID)
	if err != nil {
		return Issue{}, err
	}
	issue.ProjectRef = ref
	if _, orgSlug, err := projectScopeForGroup(ctx, db, issue.ID); err == nil {
...
func apiEventFromWebEvent(row store.WebEvent) *Event {
	resolvedFrames, unresolvedFrames := nativeFrameCounts(row.NormalizedJSON)
...
func nativeFrameCounts(raw string) (int, int) {
	if !strings.Contains(raw, `"instruction_addr"`) &&
		!strings.Contains(raw, `"debug_id"`) &&
		!strings.Contains(raw, `"package"`) {
		return 0, 0
	}
	return normalize.CountNativeFrames(raw)
}
