...
type stubColumnarLogSink struct {
	facts []ColumnarLogFact
}

func (s *stubColumnarLogSink) UpsertLogs(_ context.Context, facts []ColumnarLogFact) error {
...
func TestProjectorSyncLogsWritesColumnarSink(t *testing.T) {
	ctx := context.Background()
	source := openProjectorSourceDB(t)
	seedProjectorSource(t, source)
	bridge := openMigratedTelemetryTestDatabase(t)

	sink := &stubColumnarLogSink{}
	projector := NewProjector(source, bridge)
	projector.SetColumnarLogSink(sink)
	if err := projector.SyncFamilies(ctx, Scope{OrganizationID: "org-1", ProjectID: "proj-1"}, FamilyLogs); err != nil {
		t.Fatalf("SyncFamilies: %v", err)
	}
	if len(sink.facts) != 1 {
		t.Fatalf("len(sink.facts) = %d, want 1", len(sink.facts))
	}
	if sink.facts[0].EventID != "evt-log" || sink.facts[0].Logger != "api" {
		t.Fatalf("unexpected columnar log fact: %+v", sink.facts[0])
	}
}
