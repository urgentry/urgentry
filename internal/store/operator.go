...
func BuildOperatorFleetNodes(runtime OperatorRuntime) []OperatorFleetNode {
	role := strings.TrimSpace(runtime.Role)
	if role == "" {
		role = "unknown"
	}
	id := firstNonEmpty(runtime.NodeID, runtime.PodName, runtime.Hostname, role)
	return []OperatorFleetNode{{
		ID:           id,
		Role:         role,
		Env:          runtime.Env,
		...
		PodNamespace: runtime.PodNamespace,
	}}
}
