...
func TestBuildServerReturnsValidationError(t *testing.T) {
	db, err := sqlite.Open(t.TempDir())
	if err != nil {
		t.Fatalf("sqlite.Open: %v", err)
	}
	...
	_, err = BuildServer("api", config.Config{}, Deps{DB: db})
	if err == nil || !strings.Contains(err.Error(), "auth store") {
		t.Fatalf("BuildServer error = %v, want auth store requirement", err)
	}
}
