...
func (s *AdminStore) ListUsers(ctx context.Context, orgID string, startIndex, count int, filter string) ([]scimcore.UserRecord, int, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT u.id,
		        ...
		        u.email, u.display_name, u.is_active, u.created_at, u.updated_at
		   FROM organization_members m
		   JOIN users u ON u.id = m.user_id
		  WHERE m.organization_id = $1`+where+`
...
func (s *AdminStore) CreateUser(ctx context.Context, orgID string, user scimcore.UserRecord) (*scimcore.UserRecord, error) {
	...
	if err := postgresUpsertSCIMExternalUser(ctx, tx, orgID, userID, user.ExternalID, user.DisplayName); err != nil {
		return nil, err
	}
...
func (s *AdminStore) PatchUser(ctx context.Context, orgID, userID string, ops []scimcore.PatchOp) (*scimcore.UserRecord, error) {
	...
	if err := postgresUpsertSCIMExternalUser(ctx, tx, strings.TrimSpace(orgID), updated.ID, updated.ExternalID, updated.DisplayName); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
		return nil, err
...
func (s *AdminStore) DeleteUser(ctx context.Context, orgID, userID string) (bool, error) {
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT m.id FROM organization_members m WHERE m.organization_id = $1 AND m.user_id = $2`,
		orgID, userID,
	...
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM external_users WHERE org_id = $1 AND user_id = $2 AND provider = 'scim'`,
		orgID, userID,
	); err != nil {
		return false, err
	}
...
func (s *AdminStore) FindOrCreateSAMLUser(ctx context.Context, orgID string, user auth.SAMLUser) (*auth.User, error) {
	...
	if rec == nil {
		...
		if err := row.Scan(&existing.ID, &existing.Email, &existing.DisplayName); err != nil {
			return nil, err
		}
		return &existing, nil
	}
...
func scanPostgresSCIMUser(scanner postgresSCIMScanner) (scimcore.UserRecord, error) {
	...
	if err := scanner.Scan(&rec.ID, &externalID, &rec.Email, &rec.DisplayName, &rec.Active, &rec.CreatedAt, &rec.UpdatedAt); err != nil {
...
func postgresUpsertSCIMExternalUser(ctx context.Context, tx *sql.Tx, orgID, userID, externalID, externalName string) error {
	orgID = strings.TrimSpace(orgID)
	userID = strings.TrimSpace(userID)
	...
	if externalID == "" {
		_, err := tx.ExecContext(ctx, `DELETE FROM external_users WHERE org_id = $1 AND user_id = $2 AND provider = 'scim'`, orgID, userID)
		return err
	}

	var existingID string
	err := tx.QueryRowContext(ctx,
		`SELECT id FROM external_users WHERE org_id = $1 AND user_id = $2 AND provider = 'scim' ORDER BY created_at DESC LIMIT 1`,
		orgID, userID,
	).Scan(&existingID)
	...
	if err == sql.ErrNoRows {
		_, err = tx.ExecContext(ctx,
			...
			generateID(), orgID, userID, externalID, externalName, time.Now().UTC(),
		)
		return err
	}
	_, err = tx.ExecContext(ctx,
		`UPDATE external_users SET external_id = $1, external_name = $2 WHERE id = $3`,
...
func postgresSCIMFilter(raw string) (string, []any, bool) {
	...
	case "externalid":
		return ` AND EXISTS (SELECT 1 FROM external_users eu WHERE eu.org_id = m.organization_id AND eu.user_id = u.id AND eu.provider = 'scim' AND eu.external_id = $2)`, []any{filter.Value}, true
	default:
		return "", nil, false
	}
}
