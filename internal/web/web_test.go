...
func TestNewHandlerReturnsValidationError(t *testing.T) {
	dataDir := t.TempDir()
	db, err := sqlite.Open(dataDir)
	if err != nil {
		t.Fatalf("sqlite.Open: %v", err)
	...
	if err == nil || !strings.Contains(err.Error(), "web and query services") {
		t.Fatalf("NewHandler error = %v, want query service requirement", err)
	}
}
...
func TestMonitorsPage(t *testing.T) {
	...
	if !strings.Contains(body, "due in") && !strings.Contains(body, "overdue by") {
		t.Fatalf("expected monitor next check-in detail in body, got %s", body)
	}
}
...
func TestCronsAliasRedirectsToMonitors(t *testing.T) {
	srv, db := setupTestServer(t)
	defer srv.Close()
	...
	resp, err := client.Get(srv.URL + "/crons/")
	if err != nil {
		t.Fatalf("GET /crons/: %v", err)
	}
	if resp.StatusCode != http.StatusSeeOther || resp.Header.Get("Location") != "/monitors/" {
		t.Fatalf("GET /crons/ status=%d location=%q", resp.StatusCode, resp.Header.Get("Location"))
	}
	resp.Body.Close()

	resp, err = client.Get(srv.URL + "/crons/" + monitor.ProjectID + "/" + monitor.Slug + "/")
	if err != nil {
		t.Fatalf("GET /crons detail: %v", err)
	}
	want := "/monitors/" + monitor.ProjectID + "/" + monitor.Slug + "/"
	if resp.StatusCode != http.StatusSeeOther || resp.Header.Get("Location") != want {
		t.Fatalf("GET /crons detail status=%d location=%q want=%q", resp.StatusCode, resp.Header.Get("Location"), want)
	}
	resp.Body.Close()
}
