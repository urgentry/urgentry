	...
	return ScaleValidationGate{
		Checks: []GateEntry{
			...
			},
			{
				...
				Command:    "deploy/compose/upgrade.sh",
			...
			},
			{
				...
				Command:    "go test ./internal/sqlite -run QueryGuard",
			...
			},
			{
				...
				Command:    "deploy/compose/drills.sh bridge-rebuild",
				Artifact:   "eval/reports/selfhosted/scorecard.md",
				SuccessBar: "bridge rebuild and lag-recovery drills stay green",
			},
			{
				Check:      GateCheckKubernetesSmoke,
				Command:    "deploy/k8s/smoke.sh",
				Artifact:   "deploy/k8s/secret.yaml",
